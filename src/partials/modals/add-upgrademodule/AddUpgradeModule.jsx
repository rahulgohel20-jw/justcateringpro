...
import { UpdatePlanById } from "@/services/apiServices";
import { AddUpgrademodule } from "../../../services/apiServices";

export default function AddUpgradeModule({
  open,
  onCancel,
  onSuccess,
  editPlan,
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      isConfig: formData.isPopular,
      upgradedModuleFeatures: formData.features
        .filter((f) => f.trim() !== "")
        .map((f, index) => ({
          id: editPlan?.upgradedModuleFeatures?.[index]?.id || -1,
          featureText: f,
        })),
    };
    ...
    try {
      let res;
      if (editPlan) {
        res = await AddUpgrademodule(payload);
      } else {
        res = await AddUpgrademodule(payload);
      }
      ...
      if (data?.success === true) {
        await Swal.fire({
          title: "Success!",
          text: data?.msg || "Module saved successfully!",
          icon: "success",
        ...
        Swal.fire("Error!", data?.msg || "Failed to save module", "error");
      }
    } catch (error) {
            ...
            >
              ...
              <div className="flex items-center justify-between p-5 border-b border-gray-200">
                <h2 className="text-xl font-semibold text-gray-800">
                  {editPlan ? "Edit Module " : "Add New Module"}
                </h2>{" "}
                <button
                  onClick={onCancel}
