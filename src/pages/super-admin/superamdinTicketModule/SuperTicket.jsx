...
import ClientDetailPage from "./ClientDetilslist/ClientDetailsPage";
...
// ── Mock data — each row maps to one client query ────────────────────────────
// Note: "module", "dept", "assignee", "from", "to", "time", "assigned", "completed"
// are the field names ClientDetailsPage expects.
const mockData = [
  {
    id: 1,
...
// ── Cell renderers for priority / status ─────────────────────────────────────
const cellRenderers = {
  priority: ({ row }) => (
    <span
...
const SuperTicket = () => {
  ...
  const columns = buildColumns({
    ...
    onClientClick: (row) => setSelectedClient(row),
  });
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-lg transition self-start sm:self-auto"
        >
          <svg
            className="w-4 h-4"
            fill="none"
