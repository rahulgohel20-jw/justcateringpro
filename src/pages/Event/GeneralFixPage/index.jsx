...
const GeneralFixPage = () => {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      ...
      try {
        ...
        setEventData(response?.data?.data?.["Event Details"]?.[0] || null);
      } catch (error) {
  ...
  const fetchGeneralFixItems = async (categoryId) => {
    ...
    try {
      ...
      setItems(
        Array.isArray(allocationItems)
          ? allocationItems.map((item, index) => {
              const finalQty = Number(item.finalQty ?? item.qty ?? 0);
              const total = Number(item.totalprice ?? item.total ?? 0);
  ...
  useEffect(() => {
    const fetchGeneralFixCategories = async () => {
      ...
      try {
        const userId = localStorage.getItem("userId");
        const response = await GetrawMaterialCatIdbytypeid(1, userId);
        const categories =
        ...
        setTabs(dynamicTabs);
        setActiveTab(dynamicTabs[0]?.value || null);
        if (dynamicTabs[0]?.categoryId) {
          await fetchGeneralFixItems(dynamicTabs[0].categoryId);
        }
      } catch (error) {
        console.error("Error fetching general fix categories:", error);
        setTabs([]);
        setActiveTab(null);
        setItems([]);
      }
  ...
  const handleTabSwitch = async (tab) => {
    ...
    await fetchGeneralFixItems(tab.categoryId);
  };
  ...
  const handleSaveFromSidebar = (updatedRow) => {
    setItems((previous) =>
      previous.map((item) =>
        getRowId(item) === getRowId(updatedRow)
          ? { ...item, ...updatedRow }
          : item,
      ),
    );
  ...
  const handleSave = async () => {
    if (!eventId || !activeTab || !hasChanges) return;

    setTableLoading(true);
    try {
      const payload = {
        eventFunctionId:
          items[0]?.eventRawMaterialFunctions?.[0]?.eventFunctionId || 0,
        eventId: Number(eventId),
        rawMaterialCategoryId: Number(activeTab),
        eventRawMaterial: items.map((item) => ({
          eventRawMatFunctions: item.eventRawMaterialFunctions || [],
          extraItem: item.isExtraItem ? item.material : "",
          finalQty: Number(item.finalQty) || 0,
          place: item.place || "",
          qty: Number(item.qty) || 0,
          rawMaterialId: item.isExtraItem ? null : item.rawMaterialId || null,
      ...
      if (
        ...
        await fetchGeneralFixItems(activeCategory?.categoryId);
        Swal.fire({
          icon: "success",
          title: "Saved",
      ...
      console.error("Error saving General Fix data:", error);
      Swal.fire({
        icon: "error",
        title: "Save Failed",
  ...
  const functionVenues = (eventData?.eventFunctions || [])
    .map((eventFunction) => {
      ...
      return value ? { value, label: value, id: venue?.id || value } : null;
    })
    .filter(
  ...
  const eventVenue = functionVenues[0]?.label || eventData?.venue?.nameEnglish || "-";
    ...
    <Container>
      <div className="gap-2 mb-3">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-6">
            <h2 className="text-xl text-black font-semibold">General Fix</h2>
            <div className="flex flex-wrap gap-2">
              {permMenuPlanning.view && <button type="button" onClick={() => navigate(`/menu-preparation/${eventId}`)} className={stepButtonClass}><i className="ki-filled ki-menu text-primary text-md"></i>2. Menu Planning</button>}
              {permMenuExecution.view && <button type="button" onClick={() => navigate(`/menu-allocation/${eventId}`)} className={stepButtonClass}><i className="ki-filled ki-gift text-primary text-md"></i>3. Menu Execution</button>}
              {permRawMaterial.view && <button type="button" onClick={() => navigate(`/raw-material-allocation/${eventId}`)} className={stepButtonClass}><i className="ki-filled ki-box text-primary text-md"></i>4. Raw Material Distribution</button>}
              {permAgencyDistribution.view && <button type="button" onClick={() => navigate(`/labour-and-other-management/${eventId}`)} className={stepButtonClass}><i className="ki-filled ki-gift text-primary text-md"></i>5. Agency Distribution</button>}
              {permPerDishCosting.view && <button type="button" onClick={() => navigate(`/dish-costing/${eventId}`)} className={stepButtonClass}><i className="ki-filled ki-grid text-primary text-md"></i>6. Per Dish-costing</button>}
            </div>
            <button type="button" onClick={() => navigate("/")} className="btn border border-gray-300 text-gray-700 bg-white font-semibold hover:bg-gray-100">
      ...
      <div className="card min-w-full rtl:[background-position:right_center] [background-position:right_center] bg-no-repeat bg-[length:500px] user-access-bg mb-5">
        <div className="flex flex-wrap items-center justify-between p-4 gap-3">
          <div className="flex items-center gap-3"><i className="ki-filled ki-calendar-tick text-success text-lg"></i><div className="flex flex-col"><span className="text-sm">Event ID:</span><span className="text-sm font-medium text-gray-900 underline">{eventData?.eventNo || "-"}</span></div></div>
          <div className="flex items-center gap-3"><i className="ki-filled ki-user text-success text-lg"></i><div className="flex flex-col"><span className="text-sm">Party Name:</span><span className="text-sm font-medium text-gray-900">{eventData?.party?.nameEnglish || "-"}</span></div></div>
          <div className="flex items-center gap-3"><i className="ki-filled ki-geolocation-home text-success text-lg"></i><div className="flex flex-col"><span className="text-sm">Event Name:</span><span className="text-sm font-medium text-gray-900">{eventData?.eventType?.nameEnglish || "-"}</span></div></div>
          <div className="flex items-center gap-3"><i className="ki-filled ki-calendar-tick text-success text-lg"></i><div className="flex flex-col"><span className="text-sm">Event Date &amp; Time:</span><span className="text-sm font-medium text-gray-900">{eventData?.eventStartDateTime || "-"}</span></div></div>
          <div className="w-full h-0"></div>
          <div className="flex items-center gap-3"><i className="ki-filled ki-calendar-tick text-success text-lg"></i><div className="flex flex-col"><span className="text-sm">Event Venue:</span><span className="text-sm font-medium text-gray-900">{eventVenue}</span></div></div>
        </div>
      </div>
        ...
        {tabs.length === 0 ? (
          ...
          tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium border border-gray-200 ${activeTab === tab.value ? "bg-primary text-white" : "bg-gray-50 text-gray-600"}`}
            >
              {tab.label}
            </button>
          ))
      ...
      <div className="flex justify-between mb-4">
        <div className="flex w-fit items-center gap-3">
          <div className="filItems relative">
            ...
            <input className="input pl-8" placeholder="Search Items" type="text" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow border border-gray-200">
            ...
            <tbody>
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item, index) => {
                  ...
                  return (
                    <tr key={rowId || index} className="border-b border-gray-200">
                      <td className="px-4 py-3">{item.displayId}</td>
                      <td className="px-4 py-2 text-xs text-gray-700 truncate" title={item.material}>
                        {item.material}
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {item.date ? dayjs(item.date).format("DD/MM/YYYY hh:mm A") : "-"}
                      </td>
                      <td className="px-4 py-3 text-xs text-gray-600 truncate" title={item.remarksEnglish || ""}>
                        <div className="flex items-center gap-2">
                          <span>{item.remarksEnglish || "-"}</span>
                          <button type="button" className="w-6 h-6 flex items-center justify-center rounded-full bg-blue-100 text-blue-600">
                            <i className="ki-filled ki-notepad-edit text-xs"></i>
                          </button>
                        </div>
                      </td>
                      <td className="px-4 py-3">{Number(item.total || 0).toFixed(2)}</td>
        ...
        <div className="flex justify-between items-center px-4 py-4 border-t bg-gray-50">
          <div className="text-sm font-medium">
            Total Price: <span className="font-semibold text-blue-700">{totalPrice.toFixed(2)}</span>
          </div>
          <button
            type="button"
            onClick={handleSave}
            disabled={!hasChanges || tableLoading}
            className={`text-sm px-5 py-2 rounded-md ${hasChanges && !tableLoading ? "bg-primary text-white" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
          >
            Save
          </button>
        </div>
