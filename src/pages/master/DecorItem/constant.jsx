...
export const columns = (onEdit, onDelete, onStatus, permissions) => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const imgSrc = row.original.imagesPath || row.original.imagePath || "";
      return imgSrc ? (
        <img
          src={imgSrc}
  ...
  {
    accessorKey: "slogan",
    header: <FormattedMessage id="COMMON.SLOGAN" defaultMessage="Slogan" />,
    meta: {
      headerClassName: "w-[12%]",
      cellClassName: "w-[12%]",
    },
    cell: ({ row }) => (
      <span className="line-clamp-2">{row.original.slogan || "-"}</span>
    ),
  },
  {
    accessorKey: "decoreMainCategoryId",
    header: (
      <FormattedMessage
        id="DECOR_ITEM.CATEGORY"
        defaultMessage="Category_ID"
      />
    ),
    meta: {
      headerClassName: "w-[8%]",
