...
import { columns } from "../alluser/democonstant";
import {
  getAllByRoleIdData,
  LoginWithOtp,
  transfermember,
...
const DemoUsers = () => {
  const permissions = usePermission("Demo Member");
  ...
  const handleFetchByRoleId = async () => {
    try {
      setLoading(true);
      const response = await getAllByRoleIdData(2, "demo");
  ...
  // ─── Convert: call transfermember with otp=-1 first, then open ApproveOtp modal ─
  const handleConvertClick = async (userId) => {
    try {
      setLoading(true);
      const otp = "-1";
      const res = await transfermember(otp, "demo", userId);
  ...
  return (
    <Container>
      <div className="gap-2 pb-2 mb-3">
        <Breadcrumbs items={[{ title: "Demo Members" }]} />
      </div>
        ...
        <div className="relative -webkit-overflow-scrolling-touch">
          <TableComponent
            columns={columns(
              ...
              handleAssignUser,
              handleExtendClick,
              handleConvertClick, // ← passed to columns
              permissions,
            )}
            data={filteredData}
            paginationSize={10}
      ...
      {/* Single ApproveOtp modal handles all actions including convert */}
      <ApproveOtp
