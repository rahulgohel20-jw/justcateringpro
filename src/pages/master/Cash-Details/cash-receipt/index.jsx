...
import { cashReceiptColumns } from "./constant";
...
import Addcashreceipt from "../../../../partials/modals/add-cash-account/Addcashreceipt";
...
const SuperCashReceipt = () => {
  const permissions = usePermission("CashRecipet");
  ...
  // ── Dates and active state from API only — no FE fallback ──
  ...
  useEffect(() => {
    ...
    if (activeEntry) {
      setMonth(activeEntry.month - 1); // API month is 1-based
  ...
  const fetchReceipts = async () => {
    ...
    try {
      ...
      const selectedTab = cashSubTabs.find((s) => s.key === subTab);
      const selectedId =
  ...
  useEffect(() => {
    if (subTab && !closeDateLoading && cashSubTabs.length > 0) fetchReceipts();
  }, [subTab, month, year, closeDateLoading, monthCloseDates, cashSubTabs]);
  ...
  return (
    <Container>
      <div className="">
        ...
        <div className="flex flex-wrap items-center gap-3 md:hidden mb-4">
          <select
            ...
            className="border border-gray-200 rounded-xl px-3 py-2 text-sm font-semibold text-gray-800 bg-white cursor-pointer outline-none"
          >
            {Array.from(
              { length: 10 },
        ...
        <div className="flex items-center justify-between">
          ...
          {permissions.add && (
            ...
            >
              ...
              <FormattedMessage
                id="RECEIPT.ADD_CASH_RECEIPT"
                defaultMessage="Add Cash Receipt"
              />
            </button>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <TableComponent
            columns={cashReceiptColumns(handleAction, permissions)}
            data={filteredData}
      ...
      <Addcashreceipt
        ...
        receiptDetails={modalState.data}
        type="cash"
        userId={userId}
...
export default SuperCashReceipt;
