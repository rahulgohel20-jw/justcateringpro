      ...
      <div className="fixed inset-0 z-[100]">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3 px-6 py-4 border-b border-[#e5e7eb] bg-[#f9fafb]">
            ...
            <div>
              <p className="text-[11px] uppercase tracking-wide text-[#9ca3af]">Paid</p>
              <p className="font-semibold text-red-500 mt-0.5">
                ₹{Number(vendor.totalPayable).toLocaleString("en-IN")}
              </p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-wide text-[#9ca3af]">Outstanding</p>
              <p className={`font-semibold mt-0.5 ${isSettled ? "text-green-600" : "text-orange-600"}`}>
          ...
          outstanding: (
            <span
              className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${
                isSettled ? "bg-[#dcfce7] text-green-700" : "bg-[#ffedd5] text-orange-700"
              }`}
            >
              ₹{Number(vendor.totalRemaining).toLocaleString("en-IN")}
            </span>
   ...
   const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-medium text-red-500">
            ₹{Number(row.original.totalPayable).toLocaleString("en-IN")}
          </span>
        ),
      },
      {
