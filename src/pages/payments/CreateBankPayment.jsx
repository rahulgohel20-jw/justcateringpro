...
const CreateBankPayment = ({ isOpen, onClose, onSaveSuccess, editData }) => {
...
const [isAccountMasterModalOpen, setIsAccountMasterModalOpen] = useState(false);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (editData) {
        ...
        const isEditPurchase = editData.vendorCat === "Purchase";
        ...
        setFormData({
          ...
          depositTo: editData.bankId != null ? String(editData.bankId) : "", // ← fix
          cashId: editData.cashId != null ? String(editData.cashId) : "", // ← fix
          referenceId: editData.referenceId ?? "",
          remarks: editData.remarks ?? "",
          voucherNo: editData.voucherno ?? "",
          vendorCat: editData.vendorCat ?? "",
        });
        setIsPurchase(isEditPurchase);

        // Only set event rows if NOT purchase (same logic as add mode)
        if (!isEditPurchase) {
          setEventRows([
            {
              id: Date.now(),
              eventname: editData.eventId ?? null,
  ...
  const validate = () => {
    ...
    if (formData.paymentMode === "Cash" && !formData.cashId)
      newErrors.cashId = "Cash Account is required.";
    if (formData.paymentMode !== "Cash" && !formData.depositTo)
      newErrors.depositTo = "Bank Name is required.";
    ...
    try {
      ...
      if (editData) {
        ...
        const editPayload = {
          ...
          isPayable: true,
          payAmount: isPurchase ? 0 : Number(firstRow?.payAmount) || 0,
  ...
  <div className="flex items-center gap-2">
    <Select
      ...
      value={formData.partyId ? Number(formData.partyId) : undefined}
      onChange={handleAccountSelect}
      filterOption={(input, option) =>
        option?.label?.toLowerCase().includes(input.toLowerCase())
      }
      options={[
        ...new Map(accountData.map((acc) => [acc.partyId, acc])).values(),
      ].map((account) => ({
  ...
  <ErrorMsg field="partyId" />
                  ...
                  {/* Row 2 — Payment Mode, Bank Name / Cash Account, Reference ID */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                    ...
                    {/* Cash Account — shown only when payment mode is Cash */}
                    {/* Cash Account — shown only when payment mode is Cash */}
                    ...
                    {/* Bank Name — shown when payment mode is NOT Cash */}
                    {formData.paymentMode !== "Cash" &&
...
<tbody>
  {isVendor.length > 0 ? (
    isVendor.map((item) => {
      const isBankMissing = !item.bankName || item.bankName === "-";
      const bankOrCash = isBankMissing
        ? (item.cashName?.trim() || "-")
        : item.bankName;
