...
import { X, Upload, Trash2, RefreshCw, ChevronLeft, ChevronRight } from "lucide-react";


export default function AddServiceModal({ open, onClose, onSave }) {
  ...
  useEffect(() => {
    if (open) {
      setForm({ serviceName: "", eventType: "", price: "", minGuests: "", description: "" });
    }
  }, [open]);
  ...
  const inputCls =
    "w-full h-11 rounded-lg border border-gray-300 px-3 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 bg-white";
...
<motion.div
  className="pointer-events-auto absolute top-6 bottom-6 right-6 w-[580px] max-w-[95vw] bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col"
  initial={{ x: "110%" }}
  animate={{ x: 0 }}
  exit={{ x: "110%" }}
...
>
            ...
            <div className="px-7 pt-7 pb-4">
              ...
              <p className="text-sm text-gray-500 mt-0.5">Create a new service listing</p>
            </div>

            {/* Body */}
            <div className="px-7 pb-2 flex flex-col gap-5 overflow-y-auto  no-scrollbar">
              {/* Row 1: Service Name + Event Type */}
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-gray-700">Service Name</label>
                  <input
                    className={inputCls}
                    placeholder="e.g. Luxuries Food"
              ...
              {/* Row 2: Price + Min Guests */}
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-gray-700">Price (Including GST)</label>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-gray-700">Minimum Guests</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">₹</span>
                    <input
                      className={`${inputCls} pl-7`}
              ...
              <div className="flex flex-col gap-2">
                <div>
                  <p className="text-sm font-semibold text-gray-800">Add Service Image</p>
                ...
                <div className="flex flex-col gap-2">
                  {files.map((file, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg px-3 py-2.5">
            ...
            {/* Footer */}
            <div className="px-7 py-5 flex items-center justify-between border-t border-gray-100 mt-2">
              <button
                onClick={onClose}
