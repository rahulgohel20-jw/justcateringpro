...
const AcceptSOT = () => {
  ...
  // ✅ we only need the id up front — everything else comes fresh from the API
  const sotId =
    location.state?.data?.id ?? location.state?.editData?.id ?? null;

  const [poData, setPoData]       = useState({});   // ✅ now fetched, not read from state
  ...
  // ✅ fetch the SOT fresh by id instead of trusting location.state
  const fetchSOT = async () => {
    if (!sotId) {
      console.error("[fetchSOT] No sotId found in navigation state");
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Missing SOT reference. Please go back and try again.",
    ...
    setLoading(true);
    try {
     const res = await sotidbygetbyid(sotId);
const data = res?.data?.data;
      ...
      if (!res?.data?.success || !data) {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: res?.data?.msg || "Failed to load SOT details.",
          confirmButtonColor: "#dc2626",
        });
        return;
      }
      ...
      const rows = (data.details || []).map((d, i) => ({
        ...
        availableStockUnitId:   d.availableStockUnitId,
      ...
      console.error("[fetchSOT] Error:", err);
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to load SOT details. Please try again.",
        confirmButtonColor: "#dc2626",
      });
    } finally {
      setLoading(false);
 ...
 const handleDelete = useCallback((detailId) => {
  Swal.fire({
    ...
    try {
      const res = await deleteSotDetails([detailId]); // ✅ single id, sent as an array
      ...
      if (res?.data?.success) {
        ...
        Swal.fire({
          icon: "success",
          title: "Removed!",
          text: res?.data?.msg || "Item removed successfully.",
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: res?.data?.msg || "Failed to remove item.",
          confirmButtonColor: "#dc2626",
        });
      }
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await AddAcceptSOT(payload);
  ...
  return (
    <Fragment>
      <Container>
 {loading && !poData?.id ? (
        // ✅ full-page loader — shown only on the initial fetch, before any data exists
