...
const GenerateAutoManual = () => {
  const navigate = useNavigate();
  const location = useLocation();

const [poData, setPoData] = useState(
  location.state?.data || location.state?.editData || {}
);
const [pageLoading, setPageLoading] = useState(false);
  ...
  try {
    ...
    const totalQty = rows.reduce(
      (sum, r) => sum + (Number(r.acceptedQty ?? r.qty) || 0),
      0,
    );
    const suppliersUsed = [
    ...
    const description = (() => {
      switch (status) {
        case "GENERATE_SUCCESS":
          return (
            `Auto/Manual PO Generated — SOT No: ${sotNo} | Event: ${eventName} (ID: ${eventId}) | ` +
            `Items: ${totalItems} | Total Qty: ${totalQty} | Categories: ${categoriesTouched || "N/A"} | ` +
            `Suppliers: ${suppliersUsed || "N/A"} | Generated By: ${getUserEmail() || "Unknown User"}`
          );
        case "GENERATE_PARTIAL":
          return (
            `SOT saved but PO generation FAILED — SOT No: ${sotNo} | Event: ${eventName} (ID: ${eventId}) | ` +
            `Items: ${totalItems} | Attempted By: ${getUserEmail() || "Unknown User"}`
          );
        case "GENERATE_ERROR":
          return (
            `Auto/Manual PO generation FAILED — SOT No: ${sotNo} | Event: ${eventName} (ID: ${eventId}) | ` +
            `Attempted By: ${getUserEmail() || "Unknown User"}`
          );
        default:
          return `Auto/Manual PO action performed | SOT No: ${sotNo} | Event ID: ${eventId}`;
      }
    })();

    const eventType = (() => {
      switch (status) {
        case "GENERATE_SUCCESS": return "AutoManualPO_Success";
        case "GENERATE_PARTIAL": return "AutoManualPO_Partial";
        case "GENERATE_ERROR":   return "AutoManualPO_Error";
...
useEffect(() => {
  const sotId =
    ...
    location.state?.editData?.id ||
    location.state?.sotId;

  if (sotId) {
    fetchSotById(sotId);
  } else if (poData?.details?.length) {
    // fallback: no id available, use whatever came through navigation state
    buildFromDetails(poData);
  }

  fetchAgencies();
}, []);

const buildFromDetails = (data) => {
...
const fetchSotById = async (sotId) => {
  setPageLoading(true);
  try {
  ...
  // ✅ detailId passed directly from cell — no index mismatch with search/pagination
  ...
  // ✅ detailId passed directly from cell
  ...
  // ── Save: update details first, then always call GenerateManualPO ────────────
  ...
  const payload = {
    ...
    details: allRows.map((row) => ({
      ...
      partyId:     row.supplierId ? Number(row.supplierId) : 0,
      qty:         Number(row.acceptedQty ?? row.qty),
      unitId:      Number(row.unitId),
    })),
  };

  const res1 = await AddGenerateAutoManualSOT(payload);
 

  try {
    const res2 = await GenerateManualPO(poData.id, userId);
   

    await sendLog("GENERATE_SUCCESS", allRows);
    ...
    const errMsg =
      poError?.response?.data?.message ||
      poError?.response?.data?.msg ||
      poError?.message ||
      "Unknown error";
    console.error("GenerateManualPO failed:", errMsg);

    await sendLog("GENERATE_PARTIAL", allRows);

    Swal.fire({
      icon: "warning",
      title: "Partially Done",
      text: `SOT details saved but PO generation failed: ${errMsg}`,
      confirmButtonColor: "#f59e0b",
    }).then(() => navigate(-1));
    return;
  }
} catch (e) {
  console.error("AddGenerateAutoManualSOT failed:", e?.response?.data || e?.message || e);
  ...
  Swal.fire({
    icon: "error",
    title: "Error",
    text: e?.response?.data?.message || "Failed to save SOT details. Please try again.",
    confirmButtonColor: "#dc2626",
  });
} finally {
  setIsSaving(false);
        ...
        <div className="bg-white border rounded-xl shadow-sm mb-4">
          ...
          <div className="flex justify-between items-center px-5 py-3">
            <p className="text-sm text-gray-500">
              Assign suppliers and quantities before generating the PO.
            </p>
  ...
  <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
    <TableComponent
      columns={columns(handleChange, handleDelete, agencies)}
...
export default GenerateAutoManual;