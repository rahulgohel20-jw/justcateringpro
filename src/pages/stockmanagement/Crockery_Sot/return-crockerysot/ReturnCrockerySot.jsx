...
const ReturnCrockerySOT = () => {
  const navigate = useNavigate();
  const location = useLocation();

  // Full SOT row passed via navigate state
  ...
  useEffect(() => {
    if (poData?.details?.length) {
      const rows = poData.details.map((d, i) => ({
        id:             i + 1,
        detailId:       d.id,               // used as sotDetailId in payload
        ...
        returnQty:      d.returnQty ?? 0,   // editable — starts at existing returnQty
        unit:           d.unitName,
      ...
      // Unique category tabs from rawMaterialCatId / rawMaterialCatName
      const seen = new Map();
  ...
  // ✅ detailId passed directly from cell — no index mismatch
  ...
  await AddReturnCrockerySOT(payload);
  ...
  console.error("AddReturnCrockerySOT failed:", e?.response?.data || e?.message || e);
...
export default ReturnCrockerySOT;