...
const InvoiceDetail = ({
  ...
  const columns = [
    {
      title: (
        <FormattedMessage id="SALES.INVOICE_FUNCTION" defaultMessage="Item" />
      ),
      dataIndex: "function",
      key: "function",
    },
    ...
    {
      title: (
        <FormattedMessage id="SALES.INVOICE_HSN" defaultMessage="HSN Code" />
      ), // ✅ ADD
      dataIndex: "hsn", // ✅ ADD
      key: "hsn", // ✅ ADD
    },
    {
      title: (
        <FormattedMessage id="SALES.INVOICE_PERSON" defaultMessage="Quantity" />
      ),
      dataIndex: "qty",
      key: "qty",
    },
...
Thanks!`;
  ...
  const fetchInvoiceData = async (id) => {
    ...
    try {
      ...
      if (isSuperAdmin) {
        const response = await GETSuperadmininvoicebyid(id);
        ...
        const functions = (invoice?.items || []).map((fn, index) => ({
          key: index + 1,
          function: fn?.itemName || "-",
          description: fn?.description || "-",
          qty: fn?.qty?.toString() || "0",
          tax: fn?.taxPercent ? `${fn.taxPercent}%` : "0%",
          hsn: fn?.hsnCode || "-",
          rate: fn?.rate ? Number(fn.rate).toFixed(2) : "0.00",
          amount: fn?.amount ? Number(fn.amount).toFixed(2) : "0.00",
        }));
        ...
        if (onInvoiceDataLoad) onInvoiceDataLoad(invoiceDataForParent);
        ...
        const functions = (invoice?.invoiceFunctionItems || []).map(
          (fn, index) => ({
            key: index + 1,
            ...
            person: fn?.pax || "0",
            extra: fn?.extraPax || "0",
            rate: fn?.ratePerPlate ? fn.ratePerPlate.toFixed(2) : "0.00",
            amount: fn?.amount ? fn.amount.toFixed(2) : "0.00",
          }),
        );
        setFunctionData(functions);
        ...
        const invoiceDataForParent = {
          ...
          billingname: invoice?.billingname || party?.nameEnglish || "-",
          event: event,
          salesInvoiceData: invoice?.salesInvoiceData || {},
        };
  ...
  return (
    <>
      <div className="relative scrollbar-thin::-webkit-scrollbar scrollbar-thin::-webkit-scrollbar-track .scrollbar-thin::-webkit-scrollbar-thumb:hover">
        {/* Badge Container */}
        <div className="absolute top-0 left-0 z-20 overflow-hidden w-32 h-32">
          <div
            className="absolute top-0 left-0 text-white text-center font-bold text-sm py-1"
            style={{
              width: "150px",
              ...
              backgroundColor:
                invoiceInfo.status?.toLowerCase() === "confirm"
                  ? "#16a34a"
                  : invoiceInfo.status?.toLowerCase() === "pending"
                    ? "#ca8a04"
                    : "#0066CC",
            }}
          >
            {invoiceInfo.status
              ? invoiceInfo.status.charAt(0).toUpperCase() +
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div className="p-3 sm:p-4 text-xs sm:text-sm text-gray-700 min-w-0">
              ...
              <p className="mt-4 text-xs text-gray-500 leading-relaxed">
                ...
                <br />
                {/* ✅ was: hardcoded FormattedMessage placeholder */}
                <span className="whitespace-pre-line">{invoiceInfo.tnc}</span>
              </p>
            </div>
              ...
              <div>
                ...
                <div className="flex justify-between mb-1 gap-4">
                  <span className="text-gray-500">
                    ...
                    />
                    {invoiceInfo.discountPer > 0
                      ? ` (${invoiceInfo.discountPer}%)`
                      : ""}
                  </span>
                  <span>- ₹ {invoiceInfo.discount || 0}</span>
                </div>
                <div className="flex justify-between mb-1 gap-4">
                  <span className="text-gray-500">
                    {gstInfo.taxType || "GST"}
                    {gstInfo.gstPercent > 0 ? ` (${gstInfo.gstPercent}%)` : ""}
                  </span>
                  <span>₹ {gstInfo.gstAmount || 0}</span>
                </div>
                ...
                <div className="flex justify-between mb-1 gap-4 mb-2">
                  ...
                  <span>₹ {invoiceInfo.adjustment}</span>
                </div>
                <hr />
