...
const Addinvoice = () => {
  ...
  const taxAmt =
    (subTotal - discountAmt) * ((parseFloat(taxPercent) || 0) / 100);
  const adjustmentAmt = parseFloat(adjustment) || 0;
  const grandTotal = subTotal - discountAmt + taxAmt + adjustmentAmt;
  // ── Misc ───────────────────────────────────────────────────────────────────
      ...
      prev.map((r) => {
        ...
        if (updated.itemtype === "PLAN") {
          ...
          return {
            ...updated,
            qty: String(qty),
            rate: field === "rate" ? val : updated.rate, // keep raw input while typing
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const tncValue = bankDetails
        ? `AC NO. :- ${bankDetails.accountNo?.toUpperCase() || ""}\nBRANCH :- ${bankDetails.branchName?.toUpperCase() || ""}\nIFSC CODE :- ${bankDetails.ifscCode?.toUpperCase() || ""}\nAC NAME :- ${bankDetails.accountHolderName?.toUpperCase() || ""}`
        : termsConditions || "";
      if (invoiceId) {
        ...
        formData.append("gstAmount", taxAmt.toFixed(2));
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-right px-3 py-2.5 text-[12.5px] font-semibold text-black uppercase tracking-wide w-36">
                        ...
                        <td className="px-3 py-1.5 border border-gray-200">
                          <div className="flex items-start gap-1">
                            <div className="flex-1 min-w-0">
                              {/* Item name input */}
                              <input
                                className={`w-full border rounded px-1.5 py-1 text-sm text-gray-700 outline-none hover:border-gray-200 focus:border-blue-400 focus:bg-white bg-transparent transition-colors placeholder-gray-300 ${
        ...
        <div className="grid md:grid-cols-2 gap-3 mb-5 px-8 py-5 items-stretch">
          <div className="min-w-full flex flex-col h-full">
            ...
            <TextArea
              ...
              className="flex-1 resize-none" // 🔥 important
              value={customerNotes}
